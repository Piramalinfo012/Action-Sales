...
export default function MakePaymentView({ onAddToast }: MakePaymentViewProps) {
  ...
  const loadRows = async (notify = false, silent = false) => {
    ...
    try {
      ...
      if (res.success) {
        // Only rows where PI Make Payment (AR) <> ""
        const validRows = res.data.filter(r => (r.piMakePayment || '').trim() !== '');
        setRows(validRows);
        if (notify && onAddToast) onAddToast('success', 'Refreshed', 'Make Payment list synced with sheet.');
      } else {
        if (onAddToast) onAddToast('error', 'Load Failed', res.error || 'Could not read Dispatch sheet.');
      }
    } catch (err: any) {
      if (!silent && onAddToast) onAddToast('error', 'Network Error', err.message || 'Failed to load make payment data.');
    } finally {
  ...
  const openUpdate = (row: DispatchRecord) => {
    setEditingRow(row);
    setFields({
      uploadInvoiceEwayBill: row.uploadInvoiceEwayBill || '',
      transportBill: row.transportBill || '',
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    const res = await updateMakePaymentInSheet(editingRow.rowIndex, payload);
    setIsSaving(false);

    if (!res.success) {
      if (onAddToast) onAddToast('error', 'Update Failed', res.error || 'Could not save payment data.');
      return;
    }
    if (onAddToast) onAddToast('success', 'Payment Updated', 'Make payment details saved successfully.');
    setEditingRow(null);
    loadRows(false, true);
  };
  ...
  const isConfirmed = (r: DispatchRecord) => (r.acMakePayment || '').trim() !== '';
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Make Payment To Vendor</h2>
              <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Process and record vendor payments.</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-6 px-5 py-3 rounded-2xl bg-white dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800/60 shadow-sm">
              <div className="text-center">
                <p className="text-2xl font-black text-blue-600 dark:text-blue-400 leading-none">{rows.filter(r => !(r.acMakePayment || '').trim()).length}</p>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">Awaiting</p>
              </div>
              <div className="w-px h-10 bg-slate-200 dark:bg-slate-800" />
              <div className="text-center">
                <p className="text-2xl font-black text-emerald-600 dark:text-emerald-400 leading-none">{rows.filter(r => (r.acMakePayment || '').trim()).length}</p>
      ...
      <div className="glass-card rounded-3xl p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">Items flagged for vendor payment.</p>
          </div>
        ...
        @media (max-width: 768px) {
          ...
          .responsive-mobile-table td:nth-of-type(2)::before { content: "PI Make Payment"; }
          .responsive-mobile-table td:nth-of-type(3)::before { content: "Uploaded Files"; }
          .responsive-mobile-table td:nth-of-type(4)::before { content: "Status"; }
          .responsive-mobile-table td:nth-of-type(5)::before { content: "Action"; }
        }
      `}</style>
<table className="w-full text-left border-collapse min-w-[1000px] responsive-mobile-table">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20">
                ...
                <th className="px-4 py-3.5 text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">PI Make Payment</th>
                <th className="px-4 py-3.5 text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Uploaded Files</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-slate-500 font-medium">
                    No records awaiting vendor payment.
                  </td>
                </tr>
              ) : (
                filtered.map((r, i) => (
                  <tr key={i} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{r.piMakePayment || '—'}</span>
                    </td>
                    <td className="px-4 py-4 min-w-[200px]">
                      <div className="flex flex-wrap gap-1.5">
                        {r.invoiceVendor && <a href={r.invoiceVendor} target="_blank" rel="noreferrer" className="px-2 py-1 rounded-md bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-[10px] font-bold transition-colors">Vendor Inv</a>}
                        {r.taxInvoiceWayBill && <a href={r.taxInvoiceWayBill} target="_blank" rel="noreferrer" className="px-2 py-1 rounded-md bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-[10px] font-bold transition-colors">Tax Inv & Waybill</a>}
                        {r.uploadReceiving && <a href={r.uploadReceiving} target="_blank" rel="noreferrer" className="px-2 py-1 rounded-md bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-[10px] font-bold transition-colors">Receiving</a>}
                        {r.uploadVendorCreditNote && <a href={r.uploadVendorCreditNote} target="_blank" rel="noreferrer" className="px-2 py-1 rounded-md bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-[10px] font-bold transition-colors">Vendor CN</a>}
                        {r.uploadCreditNotePPPL && <a href={r.uploadCreditNotePPPL} target="_blank" rel="noreferrer" className="px-2 py-1 rounded-md bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-[10px] font-bold transition-colors">PPPL CN</a>}
                        {!r.invoiceVendor && !r.taxInvoiceWayBill && !r.uploadReceiving && !r.uploadVendorCreditNote && !r.uploadCreditNotePPPL && (
                          <span className="text-[10px] font-medium text-slate-400">No uploads</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      {(r.acMakePayment || '').trim() ? (
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-right">
                      {!isConfirmed(r) ? (
                        ...
                        >
                          <CreditCard className="w-3.5 h-3.5 text-blue-500 group-hover/btn:text-white transition-colors" />
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-800 dark:text-white">Make Payment To Vendor</h3>
            ...
            <div className="bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-[11px]">
                  ...
                  <div><span className="block text-slate-400 font-bold uppercase tracking-wider text-[9px]">PI Make Payment</span><span className="font-bold text-slate-800 dark:text-slate-200">{editingRow.piMakePayment || '—'}</span></div>
                </div>
              </div>

              <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
                <h4 className="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">Previous Uploads</h4>
                <div className="flex flex-wrap gap-x-6 gap-y-2">
                  {editingRow.invoiceVendor ? (
                    <a href={editingRow.invoiceVendor} target="_blank" rel="noreferrer" className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"><FileText className="w-3 h-3" /> Vendor Invoice</a>
                  ) : <span className="text-[11px] font-bold text-slate-400 flex items-center gap-1"><FileText className="w-3 h-3" /> No Vendor Invoice</span>}
                  
                  {editingRow.taxInvoiceWayBill ? (
                    <a href={editingRow.taxInvoiceWayBill} target="_blank" rel="noreferrer" className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"><FileText className="w-3 h-3" /> Tax Invoice & Waybill</a>
                  ) : <span className="text-[11px] font-bold text-slate-400 flex items-center gap-1"><FileText className="w-3 h-3" /> No Tax Invoice & Waybill</span>}
                  
                  {editingRow.uploadReceiving ? (
                    <a href={editingRow.uploadReceiving} target="_blank" rel="noreferrer" className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"><FileText className="w-3 h-3" /> Receiving</a>
                  ) : <span className="text-[11px] font-bold text-slate-400 flex items-center gap-1"><FileText className="w-3 h-3" /> No Receiving</span>}
                  
                  {editingRow.uploadVendorCreditNote ? (
                    <a href={editingRow.uploadVendorCreditNote} target="_blank" rel="noreferrer" className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"><FileText className="w-3 h-3" /> Vendor Credit Note</a>
                  ) : <span className="text-[11px] font-bold text-slate-400 flex items-center gap-1"><FileText className="w-3 h-3" /> No Vendor Credit Note</span>}
                  
                  {editingRow.uploadCreditNotePPPL ? (
                    <a href={editingRow.uploadCreditNotePPPL} target="_blank" rel="noreferrer" className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"><FileText className="w-3 h-3" /> PPPL Credit Note</a>
                  ) : <span className="text-[11px] font-bold text-slate-400 flex items-center gap-1"><FileText className="w-3 h-3" /> No PPPL Credit Note</span>}
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Transport Bill</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    value={fields.makePaymentRemark}
                    onChange={(e) => setFields({ ...fields, makePaymentRemark: e.target.value })}
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
                ...
                <button type="submit" disabled={isSaving} className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl transition-all shadow-md shadow-blue-600/15 flex items-center gap-1.5 disabled:opacity-50 cursor-pointer">
                  ...
                  <span>{isSaving ? 'Saving...' : 'Submit Make Payment'}</span>
                </button>
              </div>
            </form>
          </motion.div>
