...
interface FileItem {
  ...
  url: string;
  previewUrl?: string; // Optional custom visual preview
}
...
export default function DriveFolderView({ folderId, onAddToast }: DriveFolderViewProps) {
  ...
  // Load custom files state (persistent via localStorage)
  const [files, setFiles] = useState<Record<string, FileItem[]>>(() => {
    const saved = localStorage.getItem('drive_folder_files');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('drive_folder_files', JSON.stringify(files));
  }, [files]);

  // Root Folders definition
  const folders: FolderItem[] = [
    { id: 'documents', name: 'Documents', color: 'from-blue-500 to-indigo-600', description: 'Receipt images, uploads, and media attachments.', itemCount: files.documents?.length || 0 },
    { id: 'reports', name: 'Reports', color: 'from-emerald-500 to-teal-600', description: 'Excel spreadsheets, status sheets, and audits.', itemCount: files.reports?.length || 0 },
    { id: 'contracts', name: 'Contracts', color: 'from-violet-500 to-purple-600', description: 'Legal agreements and supply conditions.', itemCount: files.contracts?.length || 0 },
    { id: 'invoices', name: 'Invoices', color: 'from-amber-500 to-orange-600', description: 'Sales invoices and payment records.', itemCount: files.invoices?.length || 0 }
  ];
  ...
  // Get active folder files filtered by search
  ...
  // Filtered folders at root level
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* List vs Grid embed togglers (Only visible inside folder) */}
          {currentFolder && (
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-slate-50 dark:bg-slate-900/40 p-4 rounded-2xl border border-slate-150 dark:border-slate-800/80">
        ...
        <div className="relative">
          ...
          <input 
            type="text"
            placeholder={currentFolder ? `Search in ${currentFolder}...` : "Search folders..."}
            value={searchQuery}
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-semibold capitalize">Adding to: {currentFolder}</p>
                </div>
              </div>
            </div>
